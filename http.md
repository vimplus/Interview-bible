...
## HTTP 1.1
HTTP/1.1引入了长连接(keep-alive)，减少了建立和关闭连接的消耗和延迟。

### HTTP 1.1 存在什么问题？
* 安全不足
* 性能不高：
    - 头部阻塞：单个TCP连接在同一时刻只能处理一个请求，意思是说，任意两个HTTP请求从开始到结束的时间里在同一个TCP连接里生命周期不能重叠（为何不能重叠？没法保证顺序）。
...
HTTP/1.1 无法保证顺序，HTTP/2基于二进制分帧层，实现了多向请求和响应：客户端和服务器可以把 HTTP 消息分解为互不依赖的帧（图 12-3），然后乱序发送，最后再在另一端把它们重新组合起来。
...
丢包的情况下。队头阻塞会导致 HTTP/2 在更容易丢包的弱网络环境下比 HTTP/1.1 更慢！


## 一个 tcp 连接能发几个 http 请求？
* HTTP 1.0：一般情况下，不支持长连接，一个 TCP 发送一个 HTTP 请求（一对一）；
* HTTP 1.1：支持了长连接，因此只要 TCP 连接不断开，便可以一直发送 HTTP 请求，没有上限（浏览器限制：6个并发）；
* HTTP 2.0：支持多用复用，一个 TCP 连接是可以并发多个 HTTP 请求，也支持长连接，无上限（一对多）；

## 与服务器建立的连接是否会在一个HTTP请求后断开？什么情况下断开？
> https://zhuanlan.zhihu.com/p/76302817
默认情况下（Connection: keep-alive）建立 TCP 连接不会断开，只有在请求报头中声明 Connection: close 才会在请求完成后关闭连接。
HTTP/1.0中发送完一个http响应就会断开。

## 一个 TCP 连接可以对应几个 HTTP 请求？
如果维持连接，一个 TCP 连接是可以发送多个 HTTP 请求的。

## 一个 TCP 连接中 HTTP 请求发送可以一起发送么？
HTTP/1.1 存在一个问题：**单个 TCP 连接在同一时刻只能处理一个请求（串行的）**。
...
有。Chrome 最多允许对同一个 Host 建立6个 TCP 连接。

## 一个浏览器窗口和一个指定的CDN域名之间，同时可以并行拉取多少张图片？为什么？
一般是6个，浏览器针对同一域名同一时间限制只能建立6个TCP连接，而一个TCP连接中（对于HTTP/1.1而言）虽然可以发送多个HTTP请求，
但是由于单个TCP连接中HTTP请求是串行的，在同一时刻只能处理一个请求，所以我们一般情况下需要采用不同的CDN域名来提高静态资源加载速度。